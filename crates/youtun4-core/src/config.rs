...
fn validate_storage_directory(path: &Path) -> Result<()> {
    ...
    if path.exists() {
        ...
        let test_file = path.join(".youtun4_write_test");
...
mod tests {
    ...
    fn test_default_playlists_directory_not_empty() {
        ...
        // Should be something like ~/Library/Application Support/youtun4/playlists on macOS
        // or ~/.local/share/youtun4/playlists on Linux
        assert!(!dir.as_os_str().is_empty());
        assert!(dir.ends_with("playlists") || dir.to_string_lossy().contains("youtun4"));
    }
    ...
    fn test_config_file_path_static() {
        ...
        // Should contain youtun4 in the path
        assert!(path.to_string_lossy().contains("youtun4"));
    }
    ...
    fn test_default_playlists_directory_is_absolute() {
        ...
        if dir != PathBuf::from(".").join("youtun4").join("playlists") {
