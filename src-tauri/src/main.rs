...
#![allow(
    ...
    reason = "Tauri main with many command registrations"
)]
// The #[tauri::command] proc macro generates code containing unreachable!() for Result type handling
#![allow(clippy::unreachable, reason = "generated by tauri::command proc macro")]
...
fn main() {
    ...
    let _logging_guard = match logging::init_auto() {
        Ok(guard) => guard,
        Err(e) => {
            // Logging hasn't been initialized yet, so stderr is the only option
            #[allow(
                clippy::print_stderr,
                clippy::semicolon_outside_block,
                reason = "logging not yet initialized; semicolon placement conflicts between lints"
            )]
            {
